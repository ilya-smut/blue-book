...
You need to create {str(question_num)} """
    query += """multiple choice questions. YOU MUST FORMAT QUESTIONS IN JSON, as follows:
...
def ask_gemini(question_num, token):
    query = gen_query(question_num)
    client = genai.Client(api_key=token)
    response = client.models.generate_content(
        model="gemini-2.0-flash-lite-preview-02-05", contents=query
    )
    response_loaded = json.loads(response.text[8:-4])
